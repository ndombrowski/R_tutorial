...
We are doing plant growth experiments and treat our plants with different microbes wondering if any affects plant growth in a postive way.
...
I hope this example is simple enough that you can see how to apply the code for your own data, as instead of growth data we might have data that looks very similar i.e.

- cell counts for microbes under different treatments
- microbial abundances across different sites
- measurements of nutrients across different sites
- ...

Let's have a look at the data first (we will care about the code later):
...
The timecourse data looks the same, we just have an extra column for the different timepoints and we only have measurements for the root length.
...
#read in data
annotation_data <- read.table("Input_files/UAP2_Annotation_table_u.txt", sep="\t", header=T,  quote = "")
...
#check for NAs across a full column
sum(is.na(timecourse$Rootlength))
