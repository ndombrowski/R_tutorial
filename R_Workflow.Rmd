...
- The file **Growth_Data.txt** contains measurements for root length and shoot gfresh weight of Arabidopsis under different growth conditions. The file **Timecourse.txt** contains measurements for root length of Arabidopsis under different growth conditions and for 5 different timepoints. Plants were grown under normal phosphorus and low P conditions. Next to control (MgCL) treatments plants were also inoculated with different strains of rhizobia to test their effect on plant growth and whether that effect depends on nutrient status. For simplicity only 1 biological experiment with 7-10 individual measurements per treatment is included.
...
We can use a similar way to connect to R on our servers, i.e ada. 
For this to work you can use the code below (just remove the # symbol).
...
The R markdown file specifies code chunks which will be executed in R (or python or bash) and plain text which will be written
to the report as is. A report is created by rendering the file in R, then the R-code is executed and the
results are merged in a pdf or html output.
...
* The R-code is embedded inbetween the **```{bash}** and **```** symbols. 
...
<p align="left">
  <img src="/Users/ninadombrowski/Desktop/WorkingDir/Notebooks/Pictures/R2.png">
</p>


An important menu-button is the **Knit** button at the top, left-hand corner of RStudio. 
...
### Other info on Rmarkdown

You can find some more information on Rmarkdown here:
...
R Markdown supports several languages, such as bash and phyton, and you can call them in the same way as R code. 
...
If you have issues, there is a lot of help online

1. Online you can check:
...
R will not remember what libraries you have loaded after you closed R. Therefore, you need to load necessary libraries everytime. Here, we will load some libraries that are usually quite helpful and it is recommended to make the libraries you load part of each of your scripts. For example like this:
...
**Notice** set the path to where ever you download the tutorial. 
...
#print variable to screen to see if we stored the right thing in our variable
wdir

#use our new  variable to set our wdir to the correct path
setwd(wdir)
```
...
Functions are build in code that we can use to make our life easier, i.e. we can calculate lengtsh of vectors, do math or do statistical analyses. 
...
#read in data
timecourse <- read.table("Input_files/Timecourse.txt", sep="\t", header=T,  quote = "")

#check the first few lines of our data
kable(head(timecourse), format='markdown')
...
```{r}
write.table(timecourse, "Output_examples/timecourse_changed.txt",  sep = "\t", row.names = T, quote =F)
```
...
Below, we introduce all these differnt types of objects.
...
Ao make the best of the R language, you'll need a strong understanding of the basic data types and data structures and how to operate on those.
...
When we print the rownames, we see that we have numbers from 1-105. When reading in a table into R it is the default behaviour how rownames are generated.
As a general rule, if you want o have other rownames, these must be unique.
...
For some thing, the order of things might matter and then we need to order the factors ourselves.
...
If we check our example dataframe, i.e. row 30, we will see that we have NAs.
Le's remove them from our dataframe for all further analyses. 
...
P_data <- timecourse_noNA[timecourse_noNA$Nutrient == "P", ]
...
growth_data_10cm <- timecourse_noNA[timecourse_noNA$Rootlength > 5, ]
dim(growth_data_10cm)

#instead of keeping rows with roots longer than 5cm we can also remove them using *!*
growth_data_no10cm <- timecourse_noNA[!timecourse_noNA$Rootlength > 5, ]
...
P_data <- timecourse_noNA[timecourse_noNA$Nutrient == "P", ]
kable(head(P_data), format = "markdown", align = 'l', booktabs = T )
```
...
Please take note, that the writer of this tutorial is no statistician ;-) .
...
table(timecourse_noNA$SampleID)
```
...
timecourse_noNA_summarized <- ddply(timecourse_noNA, .(Nutrient), summarize, Mean_RootLength = mean(Rootlength))

#view data
kable(head(timecourse_noNA_summarized), format='markdown')
```
...
timecourse_noNA_summarized <- ddply(timecourse_noNA, .(Nutrient, Timepoint, Condition), summarize, Mean_RootLength = mean(Rootlength))

#view data
kable(timecourse_noNA_summarized, format='markdown')
```
...
timecourse_noNA_summarized <- ddply(timecourse_noNA, .(Nutrient, Timepoint, Condition), summarize, RootLength = mean(Rootlength), sd = sd (Rootlength), se = sd(Rootlength) / sqrt((length(Rootlength))))
...
mapping_file <- unique(timecourse_noNA[,c("SampleID", "Nutrient", "Condition", "Timepoint")])
...
new_data_frame <- merge(timecourse_noNA, mapping_file, by = "SampleID")
...
#merge
new_data_frame <- merge(timecourse_noNA, mapping_reduced, by = "SampleID")
kable(head(new_data_frame), format='markdown')
```
...
```{r}
new_data_frame <- merge(timecourse_noNA, mapping_file[,c("SampleID", "Comment")], by = "SampleID")
kable(head(new_data_frame), format='markdown')
```
...
```{r}
#convert cm to mm
timecourse_noNA$Rootlength_mm <- timecourse_noNA$Rootlength*10
kable(head(timecourse_noNA), format='markdown')

#we can also round our data
timecourse_noNA$Rootlength_mm <- round(timecourse_noNA$Rootlength*10, digits = 1)
kable(head(timecourse_noNA), format='markdown')
```
...
timecourse_noNA$ratio <- timecourse_noNA$Rootlength/timecourse_noNA$Rootlength_mm
kable(head(timecourse_noNA), format='markdown')
...
tapply(X = timecourse_noNA$Rootlength, INDEX = timecourse_noNA$Nutrient, FUN = mean,na.rm = TRUE)
...
tapply(timecourse_noNA$Rootlength, INDEX = list(timecourse_noNA$Nutrient, timecourse_noNA$Condition),FUN = mean, na.rm = TRUE)
```
...
growth_data <- read.table("Input_files/Growth_Data.txt", sep="\t", header=T, fill=TRUE, quote = "")
...
```{r}
x <- growth_data$Rootlength
y <- growth_data$FW_shoot_mg
...
timecourse_summary <- timecourse_noNA %>% 
    ...
    summarise(mean_root = mean(Rootlength), .groups = 'drop')

kable(head(timecourse_summary), format='markdown')
